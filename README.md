...
- **命令行 `temu-delisting`**：本 README 后面写的都是这个，适合技术背景的人自己跑
- **图形界面**：`src/temu_delisting_gui/`，账号管理/扫描/审核/下架/日志都在窗口里点，不用碰命令行。源码运行：`python -m temu_delisting_gui`；给不懂技术的同事用的话，按 `packaging/README_打包说明.md` 打包成 exe 发过去即可，同事那边只需要装好 Chrome，不用装 Python
...
`scripts/daily_scan.ps1` 会跑一次"昨天"这一天的 `scan --no-review`，省得自己算日期、敲长命令：

```powershell
.\scripts\daily_scan.ps1
```

本来想接成 Windows 计划任务自动每天跑，但实测 Task Scheduler 触发的进程没法在你能看到的那个桌面会话里弹出浏览器窗口（哪怕换成弹一个最简单的提示框也一样弹不出来），这是 Windows 计划任务这个机制本身的会话隔离限制，不是代码问题，所以放弃了自动定时，改成这个手动快捷脚本。

## 运行建议 / 风控观察期

联调过程中账号后端出现过"重复提交被拦截"这类信号，说明它在监控异常行为模式，具体拦截阈值不清楚，所以建议：

- **`apply` 保持小批次**：不要一次性对几十上百个 SPU 连续跑，先跑十来个观察一阵子
- **留意账号状态**：卖家中心里如果弹出账号异常/需要验证/限制类的提示，先停下来人工处理，不要让脚本硬撞
- **本工具不会主动重试被拦截的请求**：`chat.dismiss_already_processed_alert` 只处理"已经真实处理成功"这一种重复提示，如果账号被真的限制/风控，脚本该报错还是会报错，不会尝试绕过或伪装
